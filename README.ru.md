...
`200 OK` или `429 Too Many Requests` с пустым телом, кастомизация ответа
делается на стороне Angie через `error_page`.
...
- HTTP endpoint `GET /check` — единственная точка для Angie. Принимает
  `X-Api-Key` и `X-Real-IP`, возвращает 200 / 429.
...
Клиент ─► Angie ─auth_request─► ratelimiter ─► Redis (DB1/2/3)
                  │
                  ├─[200]─► PHP upstream
                  └─[429]─► error_page → 200 с кастомным телом
```
