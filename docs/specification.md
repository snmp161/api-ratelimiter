# ТЗ: Сервис rate limiting для API-сервера
...
Сервис ограничения частоты запросов (rate limiter) для API-сервера с несколькими версиями протокола. Работает как вспомогательный HTTP-сервис, к которому Angie обращается через механизм `auth_request` по unix-сокету или TCP. Не занимается парсингом протоколов, не формирует бизнес-ответы — только принимает решение «пускать / не пускать».
...
Angie
  │
  ├─► auth_request → unix/tcp → Rate Limiter (Go)
  │                                    │
  │                                 Redis
  │                           ┌──────────────────────────────────┐
  ...
  │                           └──────────────────────────────────┘
  │
  ├─► [200 от rate limiter] → PHP upstream (запрос передаётся без изменений)
  │
  ├─► [403 от rate limiter] → error_page в Angie → ответ 200 с телом по протоколу
...
- **Angie** — извлечение api_key/token из query-параметров (приоритет api_key), маршрутизация, формирование кастомных ответов при блокировке через `error_page 403 = @...`. Rate limiting подключается только на явно указанных location'ах.
- **Rate Limiter** — только логика подсчёта и проверки лимитов. Возвращает `200 OK` или `403 Forbidden` с пустым телом. **403 (а не 429) намеренно** — модуль `auth_request` в Angie/nginx пропускает в parent только 2xx / 401 / 403, всё остальное превращается в 500. Перерендер 403 → 200 с протокол-специфичным телом — задача Angie через `error_page`.
...
Запускается раз в `--cleanup-interval` минут. Обрабатывает обе map независимо. Счётчик считается **неактивным** если `LastRequest` старше одного слота (т.е. в текущем слоте запросов не было).
...
### `GET /check` — основной endpoint (вызывается Angie через auth_request)

**Источники api_key / token.** Сервис извлекает ключ из трёх мест в
порядке приоритета (первое непустое выигрывает):

1. Заголовок `X-Api-Key` — Angie ставит его явно через `proxy_set_header`.
2. Query-параметры `?api_key=...` или `?token=...` на самом `/check` —
   удобно когда Angie может извлечь значение и положить в URL через
   `proxy_pass http://ratelimiter/check?api_key=$client_key`.
3. **Заголовок `X-Original-URI`** — Angie передаёт сюда `$request_uri`
   ...
   `$arg_api_key` в субзапросе на ряде сборок Angie возвращает пустоту,
...
X-Real-IP:       {ip}      — IP-адрес клиента, fallback при отсутствии ключа
```
...
Тело ответа всегда пустое. Кастомизацию ответа клиенту делает Angie через `error_page`.

**Гарантия fail open:** handler `/check` оборачивается в `defer/recover`. При любой внутренней панике или ошибке сервис возвращает `200 OK` — Angie всегда получает валидный ответ, никогда 5xx. Это исключает необходимость обработки ошибок auth_request на стороне Angie.
...
предполагается проксирование через Angie с IP-allowlist'ом и/или
...
## 12. Конфигурация Angie

Готовый файл — [`configs/angie.conf`](../configs/angie.conf). Ниже приведён
тот же конфиг по разделам с пояснениями.

### Передача api_key / token в ratelimiter
...
ряде сборок Angie возвращают пустую строку. `$request_uri`, наоборот,
сохраняется в субзапросе.

Поэтому Angie не извлекает `api_key`/`token` руками, а просто передаёт
...
поэтому пути в `configs/angie.conf` и в unit совпадают.
...
SIGTERM/SIGINT
    ...
    │      В период drain'а новые запросы от Angie получат connection refused.
    │      Это неизбежно при рестарте — кратковременные 502 от Angie возможны.
    │
...
| `warn` | Redis недоступен — сработал fail open; ключ исчез из redisDB1 при cleanup (api_key, был в KnownCounters); ошибка отдельного upsert в redisDB2/redisDB3 |
| `error` | Ошибка reconnect к Redis (с деталями); паника в горутине cleanup |
...
├── configs/
│   ├── angie.conf               # образец конфига Angie
...
- Отсутствующий `X-Real-IP` при пустом ключе → запрос пропускается (fail safe)
...
- `/stubs/handler_api.php` лимитируется целиком (все action). Фильтрация по `$arg_action` — при необходимости доработка на уровне Angie.
