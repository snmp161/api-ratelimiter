...
Запускается раз в `--cleanup-interval` минут. Обрабатывает обе map независимо. Счётчик считается **неактивным** если выполнены **оба условия**:
...
location'е (см. `configs/angie.conf`). Дефолт в примере конфига —
